...
- Mojo: The AI-first python superset

### AI SDK

Benefits:

- Call AI SDK and it handles the glue code involved in using different LLMs
- Helpers for common use cases, stream text, structured outputs, tool calling, handle agents
- free OOS, LLM providers

Parts:

- Core: backend for Node
- UI: framework agnostic hooks
- RSC: server components for generative UI

Usages:

- Streaming and "plain" fetching of generated text
- Streaming and "plain" fetching of structured outputs
- Receiving data as structured output so it can be displayed in custom UI
- Sending custom system messages with a common interface
- Tool calling on the client
